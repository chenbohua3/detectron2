...
class normalized_coords_transform(object):
    ...
    def __call__(self, p: Tuple[int, int]):
        return (2 * (p[0] - self.x0) / self.w - 1, 2 * (p[1] - self.y0) / self.h - 1)


@torch.jit.script
class DensePoseOutput(object):
    def __init__(self, S, I, U, V, confidences: Dict[str, torch.Tensor]):
        """
        Args:
    ...
    @torch.jit.unused
    def _crop(self, S, I, U, V,
              bbox_old_xywh: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
              bbox_new_xywh: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]):
        """
    ...
    def hflip(self, transform_data) -> None:
        """
    ...
    def _flip_iuv_semantics_tensor(self, dp_transform_data) -> None:
        if not torch.jit.is_scripting():
    ...
    def __getitem__(self, item):
        if isinstance(item, int):
            ...
            conf_selected = {}
            for key in self.confidences.keys():
